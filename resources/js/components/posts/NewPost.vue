    ...
    <v-card flat class="mt-5">
        <div class="d-flex flex-row align-center p-3">
            <v-list-item class="pr-0">
                <router-link :to="{name : 'user-posts',params : {username  : data.user.username}}">
                    <v-list-item-avatar>
                        <v-img :src="data.user.profile_src"></v-img>
                    </v-list-item-avatar>
                </router-link>
                <v-list-item-content>
                    <v-list-item-title class="" v-html="data.title"></v-list-item-title>
                    <v-list-item-subtitle v-html="data.created_at"></v-list-item-subtitle>
                </v-list-item-content>
            </v-list-item>
            <v-spacer></v-spacer>
        ...
        <router-link :to="{name : 'post-show',params : {slug  : data.slug}}">
            <v-img max-height="300"
                   :src="data.banner_src"></v-img>
        </router-link>

        <v-card-text v-html="data.dec">
        </v-card-text>
        <v-card-actions class="mt-3">
            ...
            <v-btn icon @click="bookmarkPost(data)">
                <v-icon>
                    {{
                    data.is_bookmarked ? 'mdi-bookmark' :
