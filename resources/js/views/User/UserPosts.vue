            ...
            <v-col cols="12" md="8">
                <div class="d-flex align-center flex-row justify-between">
                    ...
                    <div class="d-flex flex-row flex-column mr-10">
                        <v-btn
                            class="mr-4"
                            :color="user.is_follow ? 'info' : 'grey'"
                            rounded
                            dark
                            small
                            @click="followUser"
                ...
                <div class="mt-15">
                    <span
                        >توسط {{ user.followers_count }} نفر دنبال می شود</span
                    >
                    <span class="mr-5"
                        >{{ user.follows_count }} نفر را دنبال می کند</span
                    >
                </div>
...
export default {
    ...
    setup(props, { emit, root }) {
        ...
        const followUser = () => {
            axios.post(`/api/follow/${user.value.username}`).then(() => {
        ...
        // created hook
        axios
            .get(`/api/user-posts/${root.$route.params.username}`)
            .then(({ data }) => {
