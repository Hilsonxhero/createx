            ...
            <v-col cols="8">
                <div class="my-15">
                    ...
                    <v-tabs class="mt-10 grey darken-3" v-model="activeTab">
                        <v-tab href="#posts" @click.stop="goTo('posts')"
                            >پست ها</v-tab
                        >
                        <v-tab href="#users" @click.stop="goTo('users')"
                            >کاربران</v-tab
                        >
                        <v-tab
                            href="#categories"
                            @click.stop="goTo('categories')"
                            >دسته بندی ها</v-tab
                        >
                    </v-tabs>
                    <v-tabs-items v-model="activeTab" class="mt-7">
                        <v-tab-item
                            value="posts"
                            v-if="activeTab === 'posts' && items.data"
                        >
                            <new-post
                                v-for="(post, index) in items.data"
                                :key="index"
                                :data="post"
                            >
                            </new-post>
                        ...
                        <v-tab-item
                            value="users"
                            v-if="activeTab === 'users' && items.data"
                        >
                                ...
                                <div class="d-flex flex-row my-6" :key="index">
                                    ...
                                    <div>
                                        <v-btn
                                            ...
                                            @click="
                                                followUser(index, user.username)
                                            "
                                            v-if="
                        ...
                        <v-tab-item
                            value="categories"
                            v-if="activeTab === 'categories' && items.data"
                        >
                            <v-chip
                                router
                                :to="{
...
export default {
    ...
    setup(props, { emit, root }) {
        ...
        const fetchNextItems = () => {
            axios.get(items.value.next_page_url).then(({ data }) => {
        ...
        const fetchItems = () => {
            if (search.value !== "") {
                axios
                    .get(`/api/search/${pageType}?q=${search.value}`)
                    .then(({ data }) => {
        ...
        const followUser = (index, username) => {
            axios.post(`/api/follow/${username}`).then(() => {
                items.value.data[index].is_follow = !items.value.data[index]
                    .is_follow;
            });
        };
